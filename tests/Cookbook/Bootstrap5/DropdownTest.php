...
use UIAwesome\Html\Core\Component\Cookbook\Bootstrap5\Dropdown\{Defaults, Language};
use UIAwesome\Html\Core\Component\{Dropdown, Item};

/**
 * Unit tests for the {@see Dropdown} component with Bootstrap5 default providers.
 */
#[Group('dropdown')]
final class DropdownTest extends TestCase
{
    public function testApplyDefaultsRendersDropdownWithToggle(): void
    {
        self::assertSame(
            <<<HTML
            <div class="dropdown">
            ...
            Dropdown::tag()
                ...
                ->render(),
            'Default provider must emit the Bootstrap5 dropdown toggle and menu.',
        );
    }

    public function testApplyLanguageRendersDropupVariant(): void
    {
        self::assertSame(
            <<<HTML
            ...
            HTML,
            Dropdown::tag()
                ->addDefaultProvider(Language::class)
                ->items(Item::tag()->label('Español')->link('/es'))
                ->id('dropdown')
                ->render(),
            'Default provider must emit the language selector dropdown layout.',
        );
    }
}
